...
const send = (request = {}, queueOptions, message) => {
	if (!request.queueName) {
		localBus.emit('error', new Error('Missing queue name from send request'))
		return
	}

	whenTransportReady(() => {
		const messageTypes = config.publishMessageTypes[request.messageType] || [request.messageType];
		const wrapped = envelope.wrap(messageTypes, message)
		transport.send(request, queueOptions, wrapped)
	})
}
...
const subscribe = function busSubscribe(subscription, callback) {
  ...
  whenTransportReady(function() {
    transport.bind(subscription, myExchangeOptions);
  });
};
...
const get = (request = {}, queueOptions = {}, callback) => {
	const myOptions = {
		noAck: true
	}
	_.merge(myOptions, request.options)

	if (!request.queueName) {
		request.queueName = config.defaultQueue.name
	}

	transport.once(request.queueName + '.message', deliver)

	localBus.once(request.queueName + '.' + request.messageType, function(message, envelope, queueName) {
		process(callback, message, envelope, queueName)
	})

  whenTransportReady(function() {
    transport.get(request, queueOptions)
  })
}
