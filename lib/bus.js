...
var init = function busInit(options) {
  ...
  transport.on(defaultQueue + '.message', deliver);
	...
	transport.on('ready',function(){
		...
		transport.on(defaultQueue + '.message', function(queueName, envelope) {
...
var request = function busRequest(request, callback) {
	...
	var env = {
		...
		"sourceAddress": "rabbitmq://" + host + ":5672/" + defaultQueue,
		"destinationAddress": "rabbitmq://" + host + "/" + requestExchange,
		"responseAddress": "rabbitmq://" + host + ":5672/" + defaultQueue,
		"messageType": [
