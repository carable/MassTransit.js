...
var process = function(callback, message, envelope, queueName, clientError){
	try {
		...
		if(envelope.requestId) {
			...
			maybeAPromise = callback(clientError, envelope);
		} else {
			//regular subscription signature
			maybeAPromise = callback(message, envelope, queueName);
		}

		//async exception handler (if handler returns a promise)
		Promise.resolve(maybeAPromise).catch(function(e) {
