...
var config = {
	...
	host: 'localhost',
	queueNames: [],
	defaultQueue: os.hostname() + '-' + Guid.raw()
};
...
var init = function busInit(options) {
  ...
  transport.on(config.defaultQueue + '.message', deliver);
	transport.on(config.defaultQueue + '.ready', function() {});
	...
	transport.on('ready',function(){
		...
		transport.on(config.defaultQueue + '.message', function(queueName, envelope) {
  ...
  transport.init(config, config.defaultQueue);
};
...
var subscribe = function busSubscribe(subscription, callback) {
	...
	{
		subscription.queueName = config.defaultQueue;
	}
...
var request = function busRequest(request, callback) {
	...
	var env = {
		...
		"sourceAddress": "rabbitmq://" + config.host + ":5672/" + config.defaultQueue,
		"destinationAddress": "rabbitmq://" + config.host + "/" + requestExchange,
		"responseAddress": "rabbitmq://" + config.host + ":5672/" + config.defaultQueue,
		"messageType": [
