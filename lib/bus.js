...
const send = (queue = {}, messageType, message) => {
	if (!queue.name) {
...
const subscribe = function busSubscribe(subscription, callback, exchangeOptions) {
...
const get = (queue = {}, messageType, options = {}, callback) => {
	const myOptions = {
		noAck: true
	}
	_.merge(myOptions, options)

	if (!queue.name) {
		queue.name = config.defaultQueue.name
	}

	localBus.on(queue.name + '.' + messageType, function(message, envelope, queueName) {
		process(callback, message, envelope, queueName)
	});

  whenTransportReady(function() {
    transport.get(queue, myOptions)
  });
}
