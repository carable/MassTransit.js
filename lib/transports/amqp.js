...
const send = async (queueName, message, queueArguments = {}) => {
	const buffer = Buffer.from(serializer.serialize(message))

	// Make sure the queue is ready before we send
	await pubChannel.assertQueue(queueName, { durable: true, arguments: queueArguments })

	await pubChannel.sendToQueue(queueName, buffer)
}
