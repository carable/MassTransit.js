...
const send = async (queue, message) => {
	...
	await pubChannel.assertQueue(queue.name, { durable: true, ...queue.options })

	await pubChannel.sendToQueue(queue.name, buffer)
}

const get = async (queue, options) => {
	// Make sure the queue is ready before we send
	await channel.assertQueue(queue.name, { durable: true, ...queue.options })

	const message = await channel.get(queue.name, options)

	if (message) {
		emitter.emit(queue.name + '.message', queue.name, serializer.deserialize(message.content.toString()))
	}
}
